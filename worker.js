...
function construireSelectionGratuite(participants, quarte8) {
  ...
  const aiNumeros = new Set((quarte8 || []).map((c) => c.numero));

  const dejaChoisis = partants.filter((p) => aiNumeros.has(p.numPmu));
  const autres = partants.filter((p) => !aiNumeros.has(p.numPmu)).sort((a, b) => cote(a) - cote(b));

  const pool = [...dejaChoisis, ...autres].slice(0, 10);
  const trie = [...pool].sort((a, b) => cote(a) - cote(b));
