...
// même temps. Un unique échec silencieux plante tout l'affichage (course du
// jour, Top 4) puisque c'est désormais la seule IA active : on retente donc
// une fois après une courte pause avant d'abandonner.
async function callDeepSeek(prompt, apiKey) {
  try {
    return await callDeepSeekUneFois(prompt, apiKey);
  } catch (e) {
    const retryable = e.status === 429 || e.status >= 500 || /délai de 55s/.test(e.message || "");
