...
async function handleCourseDuJour(request, env) {
  ...
  if (!full) {
    try {
      const [courseInfo, participantsData, performancesParNumero] = await Promise.all([
        getCourseInfo(date, reunion, course),
      ...
      if (participants.length) {
        ...
        const iaStatusActives = results.map((r, i) => ({
          ia: ["DeepSeek"][i],
          ok: r.status === "fulfilled" && !!r.value,
          raison: r.status === "rejected" ? String(r.reason?.message || r.reason || "Erreur inconnue").slice(0, 150) : null,
        }));
        // Gemini/Groq/Claude désactivés à la demande — affichés comme "inactifs"
        // dans l'admin plutôt que supprimés, pour pouvoir les rebrancher facilement.
        iaStatus = [
          ...iaStatusActives,
