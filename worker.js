...
async function verifierPrediction(env, date, reunion, course, podium) {
  ...
  try {
    ...
    // "Quarté réussi" = les 4 chevaux de l'arrivée (ou moins si la course en
    // compte moins) figurent tous dans notre sélection de 8 — c'est ce
    // booléen qui pilote le badge "vérifié" affiché sur le site.
    const quarteReussi = chevauxCorrects >= Math.min(4, podiumNumeros.length);
    ...
    return { quarteReussi, chevauxCorrects };
  } catch {
