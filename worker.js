...
async function handleAbonnementChoisir(request, env) {
  ...
  await env.DB.prepare(
    ...
    .bind(email, plan, now, expiresAt)
    .run();

  return new Response(JSON.stringify({ ok: true, plan, status: "active", expires_at: expiresAt }), { headers: corsHeaders });
}
...
async function handleAdminAbonnes(request, env) {
  const corsHeaders = { "Access-Control-Allow-Origin": "*", "Content-Type": "application/json" };
  ...
  if (!env.DB) {
    return new Response(JSON.stringify({ abonnes: [] }), { headers: corsHeaders });
  }

  const rows = await env.DB.prepare(
    `SELECT email, plan, status, started_at, expires_at FROM subscriptions ORDER BY started_at DESC LIMIT 200`
  ).all();

  return new Response(JSON.stringify({ abonnes: rows.results || [] }), { headers: corsHeaders });
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/admin/abonnes") {
      return handleAdminAbonnes(request, env);
    }
