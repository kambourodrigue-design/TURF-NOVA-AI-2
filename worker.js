...
async function callClaude(prompt, apiKey) {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 2500,
      system:
        "Tu réponds STRICTEMENT avec l'objet JSON demandé par l'utilisateur, sans aucun texte, explication, raisonnement visible ou balise markdown avant ou après. Aucune phrase d'introduction ni de conclusion.",
      // Prefill : on force le début de la réponse de Claude par "{" pour qu'il
      // ne puisse pas insérer de préambule avant le JSON (technique standard
      // pour garantir une sortie JSON pure).
      messages: [
  ...
  const data = await res.json();
  const suite = data.content?.find((b) => b.type === "text")?.text || "";
  const parsed = parseQuarteJSON("{" + suite);
