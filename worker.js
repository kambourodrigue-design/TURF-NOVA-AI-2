...
async function callClaude(prompt, apiKey) {
  ...
  // On a préfixé le tour assistant par "{" pour forcer une sortie JSON pure ;
  // Claude ne renvoie pas ce caractère initial, il faut le rajouter avant de parser.
  const parsed = parseQuarteJSON("{" + text);
...
async function handleChariowWebhook(request, env) {
  ...
  try {
    ...
    const now = Date.now();
    const dureeMs = 30 * 24 * 60 * 60 * 1000;
...
async function handleArticlePublic(env, slug) {
  ...
  try {
    const article = await env.DB.prepare(
      `SELECT slug, titre, extrait, contenu, image_url, published_at FROM articles WHERE slug = ? AND statut = 'publie'`
    )
      .bind(slug)
      .first();
    if (!article) {
...
async function handleAdminArticles(request, env) {
  ...
  if (request.method === "GET") {
    const { results } = await env.DB.prepare(
      `SELECT id, slug, titre, extrait, image_url, statut, created_at, updated_at, published_at
  ...
  if (request.method === "POST") {
    ...
    await env.DB.prepare(
      `INSERT INTO articles (slug, titre, extrait, contenu, image_url, statut, created_at, updated_at, published_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
      .bind(slug, titre, extrait, contenu, imageUrl, statut, now, now, statut === "publie" ? now : null)
      .run();
...
async function handleAdminArticleItem(request, env, id) {
  ...
  if (request.method === "PUT") {
    ...
    await env.DB.prepare(
      `UPDATE articles SET titre = ?, extrait = ?, contenu = ?, image_url = ?, statut = ?, updated_at = ?, published_at = ?
       WHERE id = ?`
    )
      .bind(titre, extrait, contenu, imageUrl, statut, now, publishedAt, id)
      .run();
...
async function handleArticlePage(request, env, url) {
  ...
  try {
    article = await env.DB.prepare(
      `SELECT titre, extrait, image_url, published_at, updated_at FROM articles WHERE slug = ? AND statut = 'publie'`
    )
      .bind(slug)
      .first();
  } catch {
