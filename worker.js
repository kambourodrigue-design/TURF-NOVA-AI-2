...
// Le nom du jockey/driver n'est pas toujours renvoyé sous le même champ selon la
// discipline (trot attelé = driver, plat/obstacle/trot monté = parfois un champ
// différent selon les courses). On tente plusieurs chemins connus avant d'abandonner,
// pour éviter d'afficher "—" alors que l'info existe sous un autre nom de champ.
function getJockeyNom(p) {
  return (
    p?.driver?.nom ||
    p?.entraineur?.nom ||
    p?.montee?.nom ||
...
// Idem pour la cote : une fois la course terminée, "dernierRapportDirect" peut être
// vide alors que "dernierRapportReference" contient encore la dernière cote connue.
function getCote(p) {
  const val = p?.dernierRapportDirect?.rapport ?? p?.dernierRapportReference?.rapport;
...
// Rattrape les prédictions dont le résultat n'a jamais été vérifié — ex : un jour où
// personne n'a visité le site après les courses, donc /api/resultats-du-jour n'a jamais
// tourné pour cette date-là. On revient chercher l'arrivée réelle avec la date historique
// de la prédiction (au lieu de todayParam()) et on complète resultats_verifies + courses_features.
// Best-effort et plafonné : une course en échec ou pas encore courue ne bloque pas les autres,
// et la limite évite de bombarder l'API PMU en un seul run.
async function rattraperVerifications(env, limite = 100) {
  if (!env.DB) return { verifiees: 0, total: 0 };

  const nonVerifiees = await env.DB.prepare(
    `SELECT p.id, p.date, p.reunion, p.course
     FROM predictions p
  ...
  const rows = nonVerifiees.results || [];
  let verifiees = 0;

  for (const row of rows) {
    try {
      const numReunion = parseInt(String(row.reunion).replace("R", ""), 10);
      const numCourse = parseInt(String(row.course).replace("C", ""), 10);
      if (!numReunion || !numCourse) continue;

      const podium = await getArrivee(row.date, numReunion, numCourse);
      if (!podium) continue; // course pas encore courue, ou arrivée pas encore publiée par le PMU

      await verifierPrediction(env, row.date, row.reunion, row.course, podium);
      verifiees++;
    } catch {
      // Une course en échec ne doit jamais bloquer le rattrapage des suivantes
    }
  }

  return { verifiees, total: rows.length };
}
...
function buildPrompt(participants, courseInfo, date) {
  const chiffreJour = chiffreDuJour(date);
  const liste = participants
    .filter((p) => !p.nonPartant)
    .map((p) => {
      const cote = getCote(p);
      const jockey = getJockeyNom(p) || "inconnu";
  ...
  return `Tu es un pronostiqueur hippique professionnel, expert des pistes françaises (plat, trot attelé/monté, obstacles). Cette course a pour discipline brute (telle que fournie par l'API) : "${courseInfo.discipline || "inconnue"}".

Adapte ton analyse à la discipline sans changer la pondération ci-dessous :
— PLAT : focus sur la pointe de vitesse et la position à la corde.
— TROT (attelé/monté) : focus sur le déferrage, les allures et la fiabilité du driver au départ.
— OBSTACLE (haies, steeple-chase) : focus sur l'aptitude aux sauts et l'endurance ; repère dans la musique les lettres indiquant chute/tombé récent ("t", "Tb") et pénalise fortement.

Pondération à appliquer pour TOUTES les disciplines :
Forme récente (musique des 5 dernières courses) : 40% | Jockey/driver et entraîneur (taux de réussite) : 20% | Position à la corde : 15% | Poids porté : 10% | Aptitude à la piste/discipline/distance : 10% | Valeur (cote vs niveau réel) : 5%
...
async function obtenirPronosticConsensus(date, reunion, course, env) {
  ...
  const prompt = buildPrompt(participants, courseInfo, date);
  ...
  if (env.DB) {
    try {
      ...
      await enregistrerFeatures(env, date, reunion, course, participants, courseInfo, consensus);
    } catch (e) {
...
// Le produit "Abonnement Prestige — 2 mois" (60 000 FCFA) doit être créé/à jour sur Chariow —
// le prix affiché et facturé vient du produit Chariow lui-même, pas de ce fichier.
...
const DUREE_PLAN_MS = {
  prestige: 60 * 24 * 60 * 60 * 1000, // 2 mois
  quinzaine: 14 * 24 * 60 * 60 * 1000, // 2 semaines
};
...
// Déclenche manuellement le rattrapage des prédictions non vérifiées (voir rattraperVerifications).
// Utile pour forcer une vérification immédiate sans attendre le cron du lendemain.
async function handleAdminRattrapageVerifications(request, env) {
  const corsHeaders = { "Access-Control-Allow-Origin": "*", "Content-Type": "application/json" };
  const admin = await requireAdmin(request, env);
  if (!admin) {
    return new Response(JSON.stringify({ error: "Accès refusé" }), { status: 403, headers: corsHeaders });
  ...
  try {
    const resultat = await rattraperVerifications(env, 200);
    return new Response(JSON.stringify(resultat), { headers: corsHeaders });
  } catch (e) {
    return new Response(JSON.stringify({ error: "Erreur rattrapage", details: e.message }), {
      status: 500,
      headers: corsHeaders,
    });
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/admin/rattrapage-verifications") {
      return handleAdminRattrapageVerifications(request, env);
    }
  ...
  // et rattrapage des prédictions non vérifiées (jours sans trafic la veille, backlog éventuel).
  async scheduled(event, env, ctx) {
    ctx.waitUntil(expirerAbonnements(env));
    ctx.waitUntil(envoyerRappelsEssai(env));
    ctx.waitUntil(rattraperVerifications(env, 200));
  },
};
