...
// Prix en FCFA (XOF) — le franc CFA (zone BCEAO) est arrimé à taux fixe à l'euro par traité :
// 1 € = 655,957 FCFA, ce n'est pas un taux de marché flottant.
const PRIX_PLANS_XOF = { turfiste: 25000, prestige: 40000 };
const TAUX_XOF_PAR_EUR = 655.957;

function xofVersEurApprox(montantXof) {
  return Math.round((montantXof / TAUX_XOF_PAR_EUR) * 10) / 10;
}
...
// Choix instantané du plan gratuit uniquement — les plans payants passent obligatoirement
// par handlePaiementInitier (MoneyFusion), pour ne jamais activer un abonnement payant sans paiement confirmé.
...
// --- Paiement MoneyFusion (Mobile Money + carte bancaire) ---
//
// IMPORTANT : construit à partir des exemples officiels publiés par MoneyFusion
// (bibliothèques PHP/JS), car leur documentation technique complète bloque la
// lecture automatisée. À tester avec un vrai petit paiement avant mise en production —
// si le webhook reçu a des noms de champs différents de ceux ci-dessous, ajuster
// handlePaiementWebhook en conséquence.
...
async function handlePaiementInitier(request, env) {
  ...
  if (!env.MONEYFUSION_API_URL) {
    return new Response(JSON.stringify({ error: "Passerelle de paiement non configurée" }), { status: 503, headers: corsHeaders });
  }
  if (!env.DB) {
    return new Response(JSON.stringify({ error: "Base de données non configurée" }), { status: 503, headers: corsHeaders });
  }
  ...
  if (!PRIX_PLANS_XOF[plan]) {
    return new Response(JSON.stringify({ error: "Plan invalide" }), { status: 400, headers: corsHeaders });
  }

  const montant = PRIX_PLANS_XOF[plan];
  const orderId = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const siteUrl = new URL(request.url).origin;

  let reponseMF;
  try {
    const res = await fetch(env.MONEYFUSION_API_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        totalPrice: String(montant),
        articles: [{ name: `Abonnement ${plan} — Turf Nova AI (1 mois)`, price: String(montant), quantity: 1 }],
        numeroSend: telephone,
        nomClient: email,
        orderId,
        returnUrl: `${siteUrl}/?paiement=retour&order=${orderId}`,
        webhookUrl: `${siteUrl}/api/paiement/webhook`,
      }),
    });
    reponseMF = await res.json();
  } catch (e) {
    return new Response(JSON.stringify({ error: "Impossible de joindre MoneyFusion", details: e.message }), { status: 502, headers: corsHeaders });
  }

  if (!reponseMF || reponseMF.statut !== true || !reponseMF.url) {
    return new Response(
      JSON.stringify({ error: reponseMF?.message || "Échec de l'initialisation du paiement" }),
      { status: 502, headers: corsHeaders }
    );
  }

  await env.DB.prepare(
    `INSERT INTO paiements (order_id, token, email, plan, montant, devise, statut, created_at)
     VALUES (?, ?, ?, ?, ?, 'XOF', 'en_attente', ?)`
  )
    .bind(orderId, reponseMF.token || null, email, plan, montant, Date.now())
    .run();

  return new Response(JSON.stringify({ ok: true, url: reponseMF.url, orderId }), { headers: corsHeaders });
}

// Reçoit la notification serveur-à-serveur de MoneyFusion et active l'abonnement si le paiement est confirmé.
// C'est CE endpoint (pas la redirection du navigateur) qui fait foi pour activer l'accès.
async function handlePaiementWebhook(request, env) {
  const corsHeaders = { "Content-Type": "application/json" };
  if (!env.DB) {
    return new Response(JSON.stringify({ ok: false }), { status: 503, headers: corsHeaders });
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ ok: false, error: "payload invalide" }), { status: 400, headers: corsHeaders });
  }

  // Les deux formes possibles vues dans la documentation MoneyFusion :
  // notification directe { tokenPay, statut, ... } ou enveloppe { data: { tokenPay, statut, ... } }
  const donnees = body.data || body;
  const token = donnees.tokenPay || donnees.token || body.token;
  const statutBrut = donnees.statut;
  const estPaye = statutBrut === "paid" || statutBrut === true || statutBrut === "success";

  if (!token) {
    return new Response(JSON.stringify({ ok: false, error: "token manquant" }), { status: 400, headers: corsHeaders });
  }

  const paiement = await env.DB.prepare(`SELECT * FROM paiements WHERE token = ?`).bind(token).first();
  if (!paiement) {
    return new Response(JSON.stringify({ ok: false, error: "paiement inconnu" }), { status: 404, headers: corsHeaders });
  }

  if (paiement.statut === "paye") {
    return new Response(JSON.stringify({ ok: true, deja_traite: true }), { headers: corsHeaders });
  }

  if (!estPaye) {
    await env.DB.prepare(`UPDATE paiements SET statut = ? WHERE token = ?`).bind("echec", token).run();
  ...
  await env.DB.batch([
    env.DB.prepare(`UPDATE paiements SET statut = 'paye', paid_at = ? WHERE token = ?`).bind(now, token),
    env.DB.prepare(
      `INSERT INTO subscriptions (email, plan, status, started_at, expires_at)
       VALUES (?, ?, 'active', ?, ?)
       ON CONFLICT(email) DO UPDATE SET plan = excluded.plan, status = 'active', started_at = excluded.started_at, expires_at = excluded.expires_at`
    ).bind(paiement.email, paiement.plan, now, expiresAt),
  ]);

  return new Response(JSON.stringify({ ok: true }), { headers: corsHeaders });
}

async function handlePaiementStatut(request, env) {
  const corsHeaders = { "Access-Control-Allow-Origin": "*", "Content-Type": "application/json" };
  if (!env.DB) {
    return new Response(JSON.stringify({ error: "Base de données non configurée" }), { status: 503, headers: corsHeaders });
  }
  ...
  if (!orderId) {
    return new Response(JSON.stringify({ error: "Paramètre 'order' requis" }), { status: 400, headers: corsHeaders });
  }

  const paiement = await env.DB.prepare(`SELECT plan, statut FROM paiements WHERE order_id = ?`).bind(orderId).first();
  if (!paiement) {
    return new Response(JSON.stringify({ error: "Paiement introuvable" }), { status: 404, headers: corsHeaders });
  }

  return new Response(JSON.stringify(paiement), { headers: corsHeaders });
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/paiement/webhook" && request.method === "POST") {
      return handlePaiementWebhook(request, env);
    }

    if (url.pathname === "/api/paiement/statut") {
      return handlePaiementStatut(request, env);
    }
