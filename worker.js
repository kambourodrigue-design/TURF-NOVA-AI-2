...
// Format international attendu, ex: +2250700000000 (indicatif pays obligatoire).
function isValidPhone(phone) {
  return /^\+[1-9]\d{7,14}$/.test(phone);
}

// --- Vérification téléphone par SMS (Twilio Verify) ---
// Nécessite 3 secrets Cloudflare : TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_VERIFY_SERVICE_SID
// (wrangler secret put TWILIO_ACCOUNT_SID, etc. — voir la doc Twilio Verify).

async function twilioVerifyEnvoyer(phone, env) {
  ...
  const data = await res.json();
  if (!res.ok) {
    throw new Error(data.message || "Échec de l'envoi du SMS");
  }
  return data;
}

async function twilioVerifyVerifier(phone, code, env) {
  const { TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_VERIFY_SERVICE_SID } = env;
  if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN || !TWILIO_VERIFY_SERVICE_SID) {
    throw new Error("Service SMS non configuré");
  }
...
// Limite d'envoi souple : 5 SMS max par numéro par heure, pour éviter qu'on abuse
// du service (chaque SMS Twilio a un coût) même si Twilio a ses propres protections.
async function handleOtpEnvoyer(request, env) {
  ...
  if (!isValidPhone(telephone)) {
    return new Response(
      JSON.stringify({ error: "Numéro invalide — format attendu : +2250700000000" }),
      { status: 400, headers: corsHeaders }
    );
  }

  const compteurCle = `otp_envois:${telephone}`;
  const compteur = parseInt((await env.CACHE_KV.get(compteurCle)) || "0", 10);
  if (compteur >= 5) {
    return new Response(
      JSON.stringify({ error: "Trop de tentatives pour ce numéro — réessaie dans une heure." }),
      { status: 429, headers: corsHeaders }
    );
  }

  try {
    await twilioVerifyEnvoyer(telephone, env);
    await env.CACHE_KV.put(compteurCle, String(compteur + 1), { expirationTtl: 60 * 60 });
...
async function handleOtpVerifier(request, env) {
  const corsHeaders = { "Access-Control-Allow-Origin": "*", "Content-Type": "application/json" };
  let body;
  try {
    body = await request.json();
  ...
  if (!isValidPhone(telephone) || !code) {
    return new Response(JSON.stringify({ error: "Numéro ou code invalide" }), { status: 400, headers: corsHeaders });
  }

  try {
    const valide = await twilioVerifyVerifier(telephone, code, env);
    if (!valide) {
      return new Response(JSON.stringify({ error: "Code incorrect ou expiré" }), { status: 400, headers: corsHeaders });
    }
    // Marqueur temporaire : le téléphone est vérifié pour les 15 prochaines minutes,
    // le temps que la personne termine son inscription.
    await env.CACHE_KV.put(`phone_verified:${telephone}`, "1", { expirationTtl: 15 * 60 });
    return new Response(JSON.stringify({ ok: true }), { headers: corsHeaders });
  } catch (e) {
    return new Response(JSON.stringify({ error: e.message }), { status: 502, headers: corsHeaders });
  }
...
async function handleSignup(request, env) {
  ...
  if (!isValidPhone(telephone)) {
    return new Response(JSON.stringify({ error: "Numéro de téléphone manquant ou invalide" }), { status: 400, headers: corsHeaders });
  }

  const telephoneVerifie = await env.CACHE_KV.get(`phone_verified:${telephone}`);
  if (!telephoneVerifie) {
    return new Response(
      JSON.stringify({ error: "Téléphone non vérifié — merci de valider le code reçu par SMS." }),
      { status: 400, headers: corsHeaders }
    );
  }
  ...
  await env.CACHE_KV.put(`user:${email}`, JSON.stringify({ hash, salt, telephone, createdAt: Date.now(), actif: true }));

  // Le code SMS ne doit servir qu'une seule fois.
  await env.CACHE_KV.delete(`phone_verified:${telephone}`);

  // Essai gratuit de 4 jours à l'inscription — limité à 1 essai par numéro de téléphone
  // ET par adresse IP. Le téléphone est le vrai frein anti-abus (une IP change à chaque
  // reconnexion mobile ; un numéro vérifié par SMS est beaucoup plus coûteux à multiplier).
  if (env.DB) {
    try {
      const ip = request.headers.get("CF-Connecting-IP") || "inconnue";
      const ipDejaUtilisee = await env.CACHE_KV.get(`essai_ip:${ip}`);
      const telephoneDejaUtilise = await env.CACHE_KV.get(`essai_telephone:${telephone}`);
      const now = Date.now();

      if (ipDejaUtilisee || telephoneDejaUtilise) {
        await env.DB.prepare(
        ...
        // Mémorisé 90 jours : passé ce délai, la même IP/numéro peut redéclencher un essai
        ...
        await env.CACHE_KV.put(`essai_telephone:${telephone}`, email, { expirationTtl: 90 * 24 * 60 * 60 });
      }
    } catch {
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/auth/otp/envoyer" && request.method === "POST") {
      return handleOtpEnvoyer(request, env);
    }

    if (url.pathname === "/api/auth/otp/verifier" && request.method === "POST") {
      return handleOtpVerifier(request, env);
    }
