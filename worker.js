...
async function callDeepSeekUneFois(prompt, apiKey) {
  ...
  try {
    res = await fetch("https://api.deepseek.com/chat/completions", {
      ...
      body: JSON.stringify({
        // deepseek-reasoner (R1) : raisonne étape par étape avant de répondre,
        // nettement plus rigoureux que deepseek-chat pour une analyse multi-critères.
        // Note : ce modèle ignore temperature/top_p, donc on ne les envoie pas.
        // max_tokens ne couvre QUE la réponse finale (après le raisonnement) — le
        // raisonnement lui-même (jusqu'à 32K tokens) n'est pas compté dedans, donc
        // pas besoin de le gonfler pour ça. 8000 = maximum autorisé par l'API.
...
// plante tout l'affichage (course du jour, Top 4) puisque c'est désormais
// la seule IA active : on retente donc une fois après une courte pause, et
// si le raisonneur échoue encore, on bascule sur "deepseek-chat" (même
// fournisseur, sans raisonnement donc beaucoup plus rapide et fiable)
// plutôt que de laisser la course du jour complètement vide.
async function callDeepSeek(prompt, apiKey) {
  try {
    return await callDeepSeekUneFois(prompt, apiKey);
  } catch (e1) {
    const retryable = e1.status === 429 || e1.status >= 500 || /délai de \d+s/.test(e1.message || "");
    if (!retryable) return await callDeepSeekChatRepli(prompt, apiKey, e1);
...
async function calculerTopCoursesDuJour(env) {
  ...
  // Plafond de 25 courses analysées pour borner le nombre d'appels IA
  // (les courses les plus proches dans le temps sont prioritaires).
  const pool = aVenir.slice(0, 25);

  // DeepSeek (deepseek-reasoner) est lent et n'encaisse pas bien de grosses
  // rafales de requêtes simultanées (429 en cascade) — désormais seule IA
  // active pour les pronostics. On borne donc la concurrence (6 courses
  // analysées en même temps max) au lieu d'envoyer les 25 d'un coup, sans
  // pour autant attendre bêtement la fin d'un lot avant de lancer le suivant.
  const CONCURRENCE_MAX = 6;
  const analyses = new Array(pool.length);
  let curseur = 0;
  async function travailleur() {
    while (curseur < pool.length) {
      const i = curseur++;
      const c = pool[i];
      try {
        analyses[i] = { status: "fulfilled", value: await obtenirPronosticConsensus(date, c.reunion, c.course, env) };
      } catch (e) {
        analyses[i] = { status: "rejected", reason: e };
      }
    }
  }
  await Promise.all(Array.from({ length: Math.min(CONCURRENCE_MAX, pool.length) }, () => travailleur()));
